...
def find_obj_defs(buf, objdefs=[]):
    ...
    pat = re.compile(r"struct\s+_(" + obj_name_pat + r")\s*{\s*" +
                     "(" + obj_name_pat + r")\s+", re.MULTILINE)
    pos = 0
    while pos < len(buf):
    ...
    pat = re.compile(r"typedef struct\s+[_\w]*\s*{\s*" +
                     "(" + obj_name_pat + r")\s+[^}]*}\s*" +
                     "(" + obj_name_pat + r")\s*;", re.MULTILINE)
    pos = 0
    while pos < len(buf):
    ...
    pat = re.compile(r"struct\s+_(" + obj_name_pat + r")Class\s*{\s*" +
                     "(" + obj_name_pat + r")Class\s+", re.MULTILINE)
    pos = 0
    while pos < len(buf):
        m = pat.search(buf, pos)
        if not m: break
    ...
    pat = re.compile(r"struct\s+_(" + obj_name_pat + r")Class\s*{\s*" +
                     r"GTypeInterface\s+", re.MULTILINE)
    pos = 0
    while pos < len(buf):
        m = pat.search(buf, pos)
        if not m: break
    ...
    pat = re.compile(r"struct\s+_(" + obj_name_pat + r")Iface\s*{\s*" +
                     r"GTypeInterface\s+", re.MULTILINE)
    pos = 0
    while pos < len(buf):
        m = pat.search(buf, pos)
    ...
    while (True):
        ...
        if (match.group() == "{"):
            ...
            if (bracket_cnt == 1):
                ...
                pos = start
            bracket_cnt -= 1
    buf = re.sub(r"^\s+", "", buf, flags=re.MULTILINE)

    # remove static and inline keywords
    buf = re.sub(r'^\s*static\s+', '', buf, flags = re.MULTILINE)
    buf = re.sub(r'^\s*inline\s+', '', buf, flags = re.MULTILINE)
    buf = re.sub(r'^\s*static\s+', '', buf, flags = re.MULTILINE) # in case of "inline static" function declaration
...
#"""
arg_split_pat = re.compile(r"\s*,\s*")
...
pointer_pat = re.compile(r'.*\*$')
func_new_pat = re.compile(r'(\w+)_new$')
